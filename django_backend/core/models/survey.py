...
class SurveyManager(models.Manager):
    def create(self, name, description, anonymous, stakeholder, method, rate=0):
        stakeholder_group = StakeholderGroup.objects.get_or_create(name=stakeholder, method=method)
        survey = Survey(name=name, description=description, rate=rate, anonymous=anonymous, stakeholder_group=stakeholder_group, method=method)
        survey.save()
        return survey
